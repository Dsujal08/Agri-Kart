  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-100 to-yellow-200 flex items-center justify-center p-6">
      <div className="max-w-5xl w-full bg-white/90 backdrop-blur-xl shadow-xl rounded-xl p-8 relative border border-green-300">
        <button
          className="absolute top-6 left-6 flex items-center gap-2 text-green-700 hover:text-green-900"
          onClick={() => navigate(-1)}
        >
          <ArrowLeft className="w-5 h-5" /> Back
        </button>

        <h2 className="text-4xl font-extrabold text-center mb-8 text-green-900">
          Product Details
        </h2>
          ...
          <div className="flex flex-col items-center">
            {selectedImage ? (
              <img
                src={selectedImage}
                alt={product.name}
                className="w-full h-96 object-cover rounded-lg shadow-lg border border-green-300"
              />
            ) : (
              ...
              <div className="flex gap-4 mt-4">
                {product.images.map((img, index) => (
                  <img
                    ...
                    className={`w-20 h-20 object-cover rounded-lg cursor-pointer transition-transform hover:scale-105 border-2 ${
          ...
          <div className="flex flex-col justify-between">
            <div>
              ...
              <p className="text-2xl font-semibold mt-2">
                <span className={product.discount ? "text-red-600" : "text-green-700"}>
                  ₹{discountedPrice}
                </span>
                {product.discount && (
                  <>
            ...
            <div className="mt-6 flex flex-col sm:flex-row gap-6 items-center">
              <div className="flex gap-4 items-center">
                <button
                  className="bg-green-300 hover:bg-green-400 w-12 h-12 text-2xl font-bold rounded-full flex items-center justify-center disabled:opacity-50"
