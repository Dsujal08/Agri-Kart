...
const Detail = () => {
  ...
  const products = productss.find((item) => item.slug === slug);
  ...
  if (!products) return <p className="text-center text-gray-500">Loading product details...</p>;

  const isOutOfStock = product.stock === 0;
  const discountedPrice = products.discount ? products.price * (1 - products.discount / 100) : products.price;

  const updateQuantity = (amount) => setQuantity((prev) => Math.max(1, Math.min(prev + amount, products.stock)));
